    ...
    <section className='py-32'>
       ...
       <article>
        ...
        <ul className='text-white opacity-75 text-sm'>
            ...
            <li className='mb-2'>{singleCrew.launches} launches</li>
            {singleCrew.status === 'active'? <li className='mb-2 capitalize text-emerald-500'>Status: {singleCrew.status}</li> : 
            <li className='text-rose-500 capitalize mb-2'>Status : {singleCrew.status}</li>}
        </ul>
        <ul className='flex items-center justify-start gap-5 mt-10'>
            <li><a href={singleCrew.wikipedia} target="_blank"
            rel='noreferrer' className='btn'>Wikipedia</a></li>
